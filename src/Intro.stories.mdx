...
Please see the [PsAccounts component documentation, to integrate to your module page](?path=/docs/integration-ps-accounts-panel).
...
- if the [PsAccounts module is not installed](?path=/docs/components-alerts-alert-account-not-installed),
- if the [PsAccounts module is not activated](?path=/docs/components-alerts-alert-account-not-enabled),
- if the [multi-shop is activated and the merchant must select a shop first](?path=/docs/components-alerts-alert-multi-store-selector),
- if the [merchant has no admin rights](?path=/docs/components-connection-panels-account--need-to-be-admin)

And then you can instantiate the [Account component](?path=/docs/components-connection-panels-account) when these cases
are managed.
...
TODO : comment intégrer les styles, et autres choses en plus des composants ?
