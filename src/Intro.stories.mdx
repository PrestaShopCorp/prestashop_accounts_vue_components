...
Please see the [PsAccounts component documentation, to integrate to your module page](?path=/docs/integration-ps-accounts-panel--connected#psaccounts-component-behavior).
This component will call every sub component that is needed, and will display your own component
...
- if the [PsAccounts module is not installed](?path=/docs/components-alerts-alert-account-not-installed--not-installed),
- if the [PsAccounts module is not activated](?path=/docs/components-alerts-alert-account-not-enabled--disabled),
- if the [multi-shop is activated and the merchant must select a shop first](?path=/docs/components-alerts-alert-multi-store-selector--multi-group),
- if the [merchant has no admin rights](?path=/docs/components-connection-panels-account--need-to-be-admin)

And then you can instantiate the [Account component](?path=/docs/components-connection-panels-account--not-connected)
