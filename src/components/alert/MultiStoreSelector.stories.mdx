...
When the shop has multistore activated with 1 or many store groups, the linking process must be
donne for each store separately. So this component will display a selector (by group then by store)
to let the user select a store.

Example of the component for 1 group and many stores inside:
...
Example of the component for many groups:
